...
export default function PostInstagramItem({
	...
	const [isLiked, setIsLiked] = useState<boolean>(
		(data as any).isLikedByAuthenticatedUser,
	);
	const [likesCount, setLikesCount] = useState<number>(
		(data as any).likesCount || 0,
	);
	...
	useEffect(() => {
		...
		apiRequestJson<PostLikeSummaryResponse>(`/posts/${id}/like`, 'GET')
			.then((serverData) => {
				setIsLiked(serverData.isLikedByAuthenticatedUser);
				setLikesCount(serverData.likesCount);
			})
			.catch((err) =>
				console.error(`Failed to sync like state for post ${id}:`, err),
			);
	}, [id]);

	useEffect(() => {
		apiRequestJson<UserDataResponse>(`/users/${authorUsername}`, 'GET')
			.then((userData) => {
				setAuthorId(userData.id);
			})
			.catch((err) =>
				console.error(
					`Failed to resolve ID for user ${authorUsername}:`,
					err,
				),
			);
	}, [authorUsername]);
	...
	useEffect(() => {
		...
		apiRequest(`/users/${authorId}/profile`, 'GET')
			.then(async (res) => {
				if (res.ok) {
					const userData =
						(await res.json()) as UserProfilePetsResponse;
					setAuthorPets(userData.pets || []);
				}
			})
			.catch((err) =>
				console.error(
					`Failed to fetch pets for user ${authorId}:`,
					err,
				),
			);
	...
	const primaryPet = primaryPetName
		...
		: null;
	const primaryPetProfileLink = primaryPetName
		? getPetProfileLink(primaryPetName)
		: null;
	...
	const handleLikeToggle = async () => {
		...
		setLikesCount((prev) =>
			expectedLikedState ? prev + 1 : Math.max(0, prev - 1),
		);

		try {
			const serverData = await apiRequestJson<PostLikeSummaryResponse>(
				`/posts/${id}/like`,
				'POST',
			);
			setIsLiked(serverData.isLikedByAuthenticatedUser);
				...
				{taggedPets.length > 1 && (
					<div className='flex flex-wrap gap-2 pt-1'>
						{taggedPets.slice(1).map((pet, idx) => {
							...
							const className =
								'rounded-full border border-[#D5D7D3] bg-[#F4F4F0] px-3 py-1 text-sm font-semibold text-[#644126] transition-colors hover:border-[#C7B8AA] hover:bg-[#FFFEFB]';
