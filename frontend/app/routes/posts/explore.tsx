...
import { ApiRequestError, apiRequestJson } from '~/data/api';
...
export default function Explore() {
	...
	const handleSearch = async (e: FormEvent) => {
		...
		try {
			...
			let jsonBody:
				| PostSearchRequest
				| WithAdvancedFiltersPostSearchRequest = {
				query: query.trim(),
			};

			if (
				showAdvanced &&
				(authorHandle.trim() || petName.trim() || postDate)
			) {
			...
			const data = await apiRequestJson<PostFeedResponse>(
				endpoint,
				'POST',
				{
					jsonBody,
			...
			if (err instanceof ApiRequestError) {
				...
				if (
					...
					(
						...
						const msg =
							errorItem.defaultMessage || errorItem.message;
					...
					parsedErrors.push((payload as { message: string }).message);
				} else if (
					payload &&
					typeof payload === 'object' &&
					'detail' in payload &&
					...
					parsedErrors.push((payload as { detail: string }).detail);
				}
			}
			...
			if (parsedErrors.length === 0) {
				parsedErrors.push(
					err instanceof Error
						? err.message
						: 'Something went wrong with the search. Please try adjusting your parameters.',
				);
			}
		...
		<LoggedContainer activeItem='explore'>
			<div className='space-y-8 animate-fadeIn pt-4 max-w-2xl mx-auto'>
				<section className='rounded-[24px] bg-[#FFFEFB] p-6 shadow-[0_12px_32px_rgba(48,51,48,0.06)] border border-[#F4F4F0] md:p-8'>
					<h1 className='text-[22px] font-bold text-[#303330] mb-4'>
						Explore Petgram
					</h1>
							...
							<div className='flex gap-2 justify-end'>
								<button
									...
									className={`rounded-full border border-[#D5D7D3] px-5 py-3 text-xs font-bold transition-colors ${
										showAdvanced
											? 'bg-[#7D5739] text-white border-[#7D5739]'
											: 'bg-[#FAF9F6] text-[#303330] hover:bg-[#E7E9E4]'
									}`}
								>
									{showAdvanced
										? 'Hide Criteria'
						...
						{showAdvanced && (
							<div className='p-5 rounded-[16px] bg-[#FAF9F6] border border-[#F4F4F0] grid grid-cols-1 md:grid-cols-3 gap-4 animate-fadeIn'>
								<div className='space-y-1.5'>
									<label className='text-xs font-bold text-[#5D605C] uppercase tracking-wider'>
										Author Handle
									</label>
									<input
										type='text'
					...
					{isLoading ? (
						<div className='space-y-6'>
							{[1, 2].map((i) => (
								<div
									key={i}
									className='h-[420px] w-full rounded-[24px] bg-[#E7E9E4] animate-pulse'
								/>
							))}
						</div>
					) : results.length > 0 ? (
						...
						(hasSearched ? (
							<div className='flex flex-col items-center justify-center rounded-[24px] bg-[#FFFEFB] p-12 shadow-[0_12px_32px_rgba(48,51,48,0.06)] border border-[#F4F4F0] text-center'>
								<div className='h-14 w-14 mb-3 flex items-center justify-center rounded-full bg-[#FAF9F6] text-xl'>
									🔍
								</div>
								<h3 className='text-base font-bold text-[#303330]'>
									No matching memories found
								</h3>
								<p className='mt-1 text-xs font-medium text-[#5D605C] max-w-xs leading-relaxed'>
									Try expanding your search query terms or
									clearing the advanced filter entries.
								</p>
							</div>
						) : (
							<div className='text-center py-12 text-sm font-semibold text-[#5D605C] tracking-wide uppercase opacity-60'>
								Type a keyword description above to find
								memories 🐾
							</div>
						))
					)}
